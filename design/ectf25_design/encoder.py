...
def derive_right(parent_key: Bytes32) -> Bytes32:
    ...
    right_key_seed = struct.pack(">8I", *([0xDEADBEEF] * 8))  # Fixed right key seed
...
def derive_left(parent_key: Bytes32) -> Bytes32:
    ...
    left_key_seed = struct.pack(">8I", *([0xC0D3D00D] * 8))  # Fixed left key seed
...
class Encoder:
    ...
    def encode(self, channel: int, frame: bytes, timestamp: int) -> bytes:
        ...
        master_key = bytes.fromhex(self.global_secrets[str(channel)])
...
def main():
    ...
    print("Packet:", repr(encoded_packet))
