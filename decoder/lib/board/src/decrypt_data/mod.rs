...
pub fn decrypt_sub(board: &mut Board, encrypted_sub: vec::Vec<u8>, key: [u8; 64]) -> Option<Subscription> {
    ...
    let key_data = decrypted_data[32..].to_vec();
    ...
    host_messaging::send_debug_message(board, &format!("device id {:x}", device_id));
    // Some(Subscription {
    //     device_id_bytes,
    //     channel_bytes,
    //     start_bytes,
