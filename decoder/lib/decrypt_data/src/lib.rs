...
pub fn decrypt_sub(encrypted_sub: vec::Vec<u8>, key: [u8; 32]) -> Option<Subscription> {
    ...
    let mut decrypted_data = Aes128CbcDec::new(key, iv).decrypt_padded_vec_mut::<NoPadding>(ciphertext).unwrap();
    let device_id_bytes = [decrypted_data[0], decrypted_data[1], decrypted_data[2], decrypted_data[3]];
    let channel_bytes = [decrypted_data[4], decrypted_data[5], decrypted_data[6], decrypted_data[7]];
